...
contract PetNFT is IPetNFT, Ownable, ERC721 {
    ...
    mapping(uint256 => uint256) public levelInfos;
    mapping(uint256 => uint256) public latestUpgradeTime;
    mapping(uint256 => uint256) public latestBreedLevel;
    ...
    function upgrade(uint256 id) public returns (uint256 level) {
        ...
        levelInfos[id] = levelInfos[id] + 1;
        level = levelInfos[id];
        latestUpgradeTime[id]  = block.timestamp;
        emit Upgraded(msg.sender,id,levelInfos[id]);
    }

    //bread
    function bread(uint256 mID, uint256 fID) external {
        require(ownerOf(mID) == msg.sender&&ownerOf(fID)==msg.sender,"EggNFT : Only owner can bread");
        require(levelInfos[mID] >= minlevel && levelInfos[fID] >= minlevel,"EggNFT : not enough level");
        require((latestBreedLevel[mID] - latestBreedLevel[mID]) >= minlevel && (latestBreedLevel[fID] - latestBreedLevel[fID]) >= minlevel, "EggNFT : not exact level");
        ...
        eggNFT.bread(msg.sender,_eggInfo);

        latestBreedLevel[mID] = levelInfos[mID];
        latestBreedLevel[fID] = levelInfos[fID];
    }
